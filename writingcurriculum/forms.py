...
class ProposalForm(forms.ModelForm):
    ...
    approved_wi     = forms.ChoiceField(label="Approved Writing Intensive Course?", choices=BINARY_CHOICES, widget=forms.RadioSelect(), help_text="This course has been approved by the appropriate department as a Writing Intensive Course.")
    workshop        = forms.ChoiceField(label="Writing Intensive Workshop", choices=BINARY_CHOICES, widget=forms.RadioSelect(), help_text="Before an instructor teaches a Writing Intensive course, she must have completed a Writing Intensive workshop. Have you?")
    permission      = forms.ChoiceField(label="Permission to Include Syllabus in WAC Archive", choices=BINARY_CHOICES, widget=forms.RadioSelect(), help_text="Do you grant the WAC Committee permission to add your syllabus to our public archive of syllabi for Writing Intensive courses?")
    ...
    def clean_when_workshop(self):
        if self.cleaned_data.get('workshop')=="No" and not self.cleaned_data.get('when_workshop'):
            raise forms.ValidationError("You must provide a date by which time you will have completed a Writing Intensive workshop. ")
