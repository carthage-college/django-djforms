...
class CourseProposal(models.Model):
    ...
    academic_term       = models.ForeignKey(GenericChoice, related_name="course_proposal_academic_term")
    day_sps             = models.CharField(verbose_name="Day or SPS", max_length=4, choices=DAY_SPS_CHOICES)
    approved_wi         = models.CharField("Approved WI Course?", help_text="This course has been approved by the appropriate department as a WI Course.", max_length=3, choices=BINARY_CHOICES)
    when_approved_wi    = models.DateField("If not, when?", help_text="If not, when will the course be approved by the department?", null=True, blank=True)
    workshop            = models.CharField("WI Workshop", max_length=3, choices=BINARY_CHOICES, help_text="Before an instructor teaches a WI course, he/she must have completed a WI workshop. Have you?")
