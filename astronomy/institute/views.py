...
def night_report(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            email = EmailMessage(("[Griffin Observatory Night Report] Submitted by: %s of %s" % (data['name'],data['organization'])), t.render(c), data['email'], recipient_list, bcc, headers = {'Reply-To': data['email'],'From': data['email']})
