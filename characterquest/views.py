...
def application_profile_form(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid() and profile_form.is_valid():
            ...
            email = EmailMessage(("CharacterQuest Application: %s %s" % (applicant.profile.user.first_name,applicant.profile.user.last_name)), t.render(c), request.user.email, recipient_list, headers = {'Reply-To': request.user.email,'From': request.user.email})
