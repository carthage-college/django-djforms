...
class VisitDayBaseForm(forms.ModelForm):
    ...
    def __init__(self,event_type,*args,**kwargs):
        ...
        qs = VisitDayEvent.objects.exclude(active=False).filter(date__gte=now).filter(event__slug=event_type).order_by("date","id")
...
class VisitDayForm(forms.ModelForm):
    ...
    def __init__(self,event_type,*args,**kwargs):
        ...
        qs = VisitDayEvent.objects.exclude(active=False).filter(date__gt=now).filter(event__slug=event_type).order_by("date", "id")
        choices = [('','---choose a date---')]
        for event in qs:
            choices.append((event.id,event))
        self.fields['date'].choices = choices
...
class NursingForm(VisitDayForm):
    class Meta:
        model = VisitDayProfile
