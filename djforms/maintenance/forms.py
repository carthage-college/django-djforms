...
try:
    ...
    TYPE_OF_REQUEST = TaggedItem.objects.get_by_model(
        GenericChoice, type_of_request_tag).filter(active=True
    )
except:
...
try:
    ...
    BUILDING_NAME = TaggedItem.objects.get_by_model(
        GenericChoice, building_name_tag).filter(active=True
    )
except:
...
class EVSForm(forms.ModelForm):
    ...
    building = forms.ModelChoiceField(
        queryset=BUILDING_NAME, label="Building Name"
    )
