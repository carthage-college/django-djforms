...
class StudyAbroadForm(forms.Form):
    ...
    recent_course = forms.CharField(max_length=50, label="Current or most recent language course taken at Carthage", help_text="(language, number, and term)")
    desired_country = forms.CharField(widget=forms.Textarea, label="Country (or countries) under consideration")
    school_program = forms.CharField(label="University/School Program abroad", help_text="(if known)", widget=forms.Textarea)
    abroad_field = forms.CharField(label="Field of study while abroad", help_text="(specific coursework desired, if any)", widget=forms.Textarea)
    ...
    previous_travel = forms.TypedChoiceField(choices=BINARY_CHOICES, widget=forms.RadioSelect, label="Previous international travel or study abroad experience")
    traveled_places = forms.CharField(max_length=100, label="If yes, where?", required=False)
    passport = forms.TypedChoiceField(choices=BINARY_CHOICES, widget=forms.RadioSelect, label="Do you have a current passport?")
